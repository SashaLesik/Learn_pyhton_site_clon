...
def extract_adt_urls(category_page_html: str | None) -> list[str]:
    ...
    for div in url_divs:
        if not div.find(name='a', class_='css-rc5s2u'):
            continue
        try:
...
def parser_adt(url: str, adv_html: str) -> Adv | None:
    ...
    try:
        ...
        # soup_product_date_of_last_visit = adv_html.find('span', class_='css-1t0qnkx')
        # print(soup_product_date_of_last_visit)
        date_of_last_visit = last_visit(adv_html)
        ...
        logger.error(f'{"_" * 51}error adt{"_" * 51}', {adv_html}, exc_info=err)
        return None
    adv = Adv(
        url=url,
...
def last_visit(adv_html: str) -> datetime:
    if adv_html.find('span', class_='css-1t0qnkx'):
        soup_product_date_of_last_visit = adv_html.find('span', class_='css-1t0qnkx').text
        return parse_last_online_date(soup_product_date_of_last_visit)

    elif adv_html.find('span', class_='css-o2wpov'):
